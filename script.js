...
function computerMove() {
    positionCount = 0;
    let c = (turn % 2 == 0) ? "white" : "black";
    let output = minimaxRoot(4, board, true, c);
    selected = output[1];
...
function minimaxRoot(depth, b, isMaxing, c) {
    ...
    for (let i = 0; i < possibleMoves.length; i++) {
        ...
        turn++;
        let value = minimax(depth - 1, applyMove(possibleMoves[i][0], copyBoard(b)), -10000, 10000, !isMaxing, c);
        turn--;
...
function minimax(depth, b, alpha, beta, isMaxing, c){
    ...
    if(isMaxing){
        ...
        for(let i = 0; i < possibleMoves.length; i++){
            selected = possibleMoves[i][1];
            turn++;
            bestMove = Math.max(bestMove, minimax(depth - 1, applyMove(possibleMoves[i][0], copyBoard(b)), alpha, beta, !isMaxing, c));
            turn--;
        ...
        for(let i = 0; i < possibleMoves.length; i++){
            selected = possibleMoves[i][1];
            turn++;
            bestMove = Math.min(bestMove, minimax(depth - 1, applyMove(possibleMoves[i][0], copyBoard(b)), alpha, beta, !isMaxing, c));
            turn--;
